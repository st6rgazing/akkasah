"""
Fast import script for ArchiveFiles EAD XML data
Imports collections without processing images (images processed on-demand)
"""
import os
import sys
...
def import_archive_data_fast(archive_files_path: str, process_images: bool = False):
    """Import all EAD files from the ArchiveFiles directory
    
    Args:
        archive_files_path: Path to ArchiveFiles directory
        process_images: If True, process images (slow). If False, skip image processing (fast).
    """
    print(f"Starting import from: {archive_files_path}")
    if not process_images:
        print("NOTE: Image processing is DISABLED for faster import.")
        print("Images will be processed on-demand when viewing collections.")
    
    # Initialize parser
    parser = EADParser()
    
    # Temporarily disable image processing if requested
    original_process_method = parser._process_image_service_url
    if not process_images:
        def skip_image_processing(url, item_title=""):
            # Return minimal data without fetching HTML
            return {
                'image_id': None,
                'full_image': url,
                'thumbnail': url,
        ...
        parser._process_image_service_url = skip_image_processing
    ...
    try:
        ...
        try:
            ...
            for idx, data in enumerate(parsed_data, 1):
                ...
                # Commit every 10 collections for better performance
                if idx % 10 == 0:
                    db.commit()
                    print(f"Progress: {idx}/{len(parsed_data)} files processed...")
            ...
            if not process_images:
                print("\nNOTE: Images will be processed on-demand when viewing collections.")
                print("To process images now, run: python import_archive_data_fast.py --process-images")
        ...
        parser._process_image_service_url = original_process_method

def main():
    """Main function to run the import"""
    import argparse
    
    parser = argparse.ArgumentParser(description='Import EAD XML files into database')
    parser.add_argument('--process-images', action='store_true', 
                       help='Process images during import (slower but completes everything)')
    parser.add_argument('--path', type=str, default='/Users/mariam/akkasah/ArchiveFiles',
                       help='Path to ArchiveFiles directory')
    
    args = parser.parse_args()
    
    archive_files_path = args.path
    ...
    # Import data
    import_archive_data_fast(archive_files_path, process_images=args.process_images)
